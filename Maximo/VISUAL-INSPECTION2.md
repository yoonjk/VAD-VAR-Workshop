...
Spanish Helper: 
Para el caso de uso de esta demostración, utilizará el tipo de entrenamiento **Detección de objetos**, porque desea entrenar un modelo que pueda analizar el estado del tanque sin confundirse con ninguna otra entidad de fondo en la imagen de la cámara entrante. Además, utilizará el tipo de modelo **R-CNN más rápido**, que está optimizado para una mayor precisión, pero requiere más potencia de cómputo y GPU que las alternativas (está bien, desea una mayor precisión).
...
Además de las opciones elegidas, Maximo Visual Inspection (MVI) incluye varios modelos para ejecutar inferencias relacionadas con la clasificación y detección. Se puede leer una breve descripción de cuándo usar cada tipo de modelo dentro de cada mosaico de opción de entrenamiento y modelo.
...
Spanish Helper: En las opciones de **Tipo de alerta**, puede habilitar Maximo Visual Inspection Edge para enviar mensajes de alerta salientes a destinatarios externos mediante la configuración de conexiones a un intermediario MQTT y el servicio de mensajería Twilio.
...
Spanish Helper: 
La pantalla de la cámara del dispositivo móvil ahora está habilitada. Si no se especificó un modelo dentro de una inspección, está en modo Recopilar: toma fotos y las carga en un conjunto de datos en IBM Maximo Visual Inspection para etiquetarlas y luego entrenar un modelo.

Cuando especifica un modelo dentro de una inspección (como lo hizo en esta demostración), está en el modo Inspeccionar, que también toma fotos, pero luego usa el modelo entrenado para ejecutar la inferencia en las imágenes y cargar los resultados en el conjunto de datos especificado en IBM Maximo Visual Inspection. .

2. Para esta demostración, ejecutará la inferencia en una _imagen de una imagen_ de un tanque. Para esto, en su computadora portátil, abra una de las imágenes del tanque descargadas en la sección Prework Step 2 y manténgala en la pantalla para el próximo paso.
3. Tome una foto de la imagen de un tanque limpio o sucio que se muestra en la pantalla de su computadora o proyector (básicamente, está tomando una imagen de una imagen en este paso).

La foto permanecerá en la pantalla de su dispositivo móvil, así que presione el botón **atrás (<)** para volver a la vista de la cámara. Estos son algunos ejemplos de lo que podría ver después de algunas inspecciones.
