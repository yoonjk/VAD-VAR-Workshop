...
3. Click on the blue **New model entry** button in the top-right of the screen to open the **New model entry** dialog form.
...
4. Select the **Default Spark & Python** environment. Note that the available versions change over time, as new versions of the runtime are released and subsequently supported by Watson Studio. At the time of writing, the environment is **Default Spark 3.2 & Python 3.9**.
