...
Cloud Pak for Data accede a los datos a través de dos formas: archivos y conexiones. Los archivos **no** se limitan al formato de valores separados por comas (csv). Por ejemplo, los datos pueden venir como archivos comprimidos en formato zip o tar.
...
```sql
CREAR TABLA testing ( col1 int, col2 int, col3 int );
```
...
```csv
col1,col2,col3 8,1,6 3,5,7 4,9,2
```
...
Una de las actividades clave del cuaderno Acceso a datos es algo llamado **Insertar en código**. Para acceder a esta función, desde un cuaderno en modo de edición, tienes que abrir la pestaña de datos. La figura de arriba tiene un círculo rojo alrededor del icono de datos. La pestaña de datos se abre a la derecha e incluye dos pestañas. Una pestaña **Archivos** y otra **Conexiones**. La figura de arriba muestra dos ejemplos de apertura del signo de intercalación junto a **Insertar para codificar**. Uno para un archivo CSV y otro para un archivo ZIP. Puede ver que las opciones son diferentes ("pandas DataFrame", "Credentials", "StreamingBody object"). Esto es lo que explorarás en el laboratorio de Notebook.

![EditNotebook](./images/34.png)

Para acceder al bloc de notas, empiece por el proyecto, en la pestaña Activos.

1.  Haga clic en los tres puntos verticales al final de la línea para **01-Laboratorio de acceso a datos**. Aparecerá un menú desplegable.
