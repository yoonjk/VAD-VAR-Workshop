...
timeToComplete: 120
updated: 2023-06-10T00:00:00.000Z
---
...
1.  Utilizando el navegador, vuelva al **Panel de Control de la UBA**.
...
3.  Ejecuta los comandos de flujo:
    ...
    ```bash
    cd /archivos de laboratorio/UBA/scripts
    ```
...
>
> Antes de proceder con un escenario de investigación del comportamiento del usuario, complete los siguientes capítulos de este laboratorio que demuestran algunas configuraciones específicas de UBA para este entorno de laboratorio.
...
4.  En la ventana Añadir nuevo conjunto de referencia a la base de datos, en el campo Nombre del conjunto de referencia escriba
    ...
    ```bash
    Categorías de BlueCoat Investigate
    ```
...
8.  En la ventana Importar datos del conjunto de referencia, haga clic en Seleccionar archivo.

9.  Localice el archivo en la siguiente ruta: **/labfiles/UBA/bluecoat.txt**

10. **Desmarque** CVS contiene cabeceras.
...
6.  Seleccione las siguientes pruebas:
    ...
    1.  **cuando el evento o eventos fueron detectados por una o más de \[estas fuentes de registro** ] Seleccione la fuente de registro **BlueCoat**.

    2.  **cuando alguna de \[estas propiedades de suceso] esté contenida en alguno de \[estos] \[conjunto(s) de referencia]**
...
7.  Seleccione de nuevo, **cuando cualquiera de \[estas propiedades de evento] esté contenida en cualquiera de \[estos conjunto(s) de referencia]**.
...
8.  Desplácese hacia abajo y seleccione la nueva regla para que sea miembro del grupo **Análisis del comportamiento del usuario**.

9.  Haga clic en **Siguiente**.

10. En la sección Acción de la regla, configure lo siguiente:

    a. Activar **Asegúrese de que el evento detectado forma parte de un delito**.

    b. Delito de índice basado en: **IP de origen**.

11. En la sección Respuesta a la regla, configure como sigue:

    a. Seleccione **Enviar nuevo evento**.

    b. Tipo de Nombre de Evento: El nombre del evento debe coincidir con el nombre de la regla, de lo contrario puede afectar al tiempo de proceso de los eventos en la UBA.

    c. Tipo de descripción del evento: **senseValue=60,senseDesc='Probando la regla BlueCoat para UBA.'**

    > Las reglas por defecto que vienen con UBA tienen el parámetro de puntuación de riesgo**(senseValue**) establecido entre 5 y 25 puntos. En el laboratorio se establece este valor más alto con fines de demostración. Sin embargo, puede adoptar puntuaciones de riesgo en sus organizaciones en función de las actividades importantes que le gustaría rastrear.
    >
    ...
    e. Categoría de bajo nivel: **Evento Sospechoso Misc**. > Las versiones anteriores de UBA requerían que la Categoría de Alto Nivel para el evento relacionado con UBA fuera **Sense**. Este ya no es el caso y usted puede escoger cualquier Categoría de Alto Nivel.

    f. Seleccione **Asegurarse de que el suceso enviado forma parte de un delito**.

    g. Ofensa de índice basada en: Nombre de **usuario**. > Sólo las infracciones indexadas por la propiedad nombre de usuario se muestran en el panel UBA. Por lo tanto, debe indexar esta infracción por el nombre de usuario para obtener el resultado de laboratorio adecuado.
...
12. Desplácese hacia abajo y asegúrese de que esta nueva regla está **activada**.

13. Haga clic en **Siguiente**.

14. Revise la configuración de las reglas y haga clic en **Finalizar**.
...
8.  Puede cerrar el banner que le informa de los cambios en la puntuación de riesgo 9. Cierre los detalles de la regla.
...
3.  Debido a que el script _playuba.sh_ genera actividad aleatoria de usuarios en QRadar, este diagrama y el diagrama de usuarios monitoreados pueden mostrar diferentes resultados en su instancia de laboratorio.
...
3.  En el panel Detalles de usuario, observe lo siguiente:
    ...
    - En el gráfico **Desglose de categorías de riesgo**, puede observar qué tipos de comportamiento de riesgo practica este usuario, incluida la categoría de comportamiento generada por la regla que ha creado Evento sospechoso misceláneo.

4.  En la parte derecha del cuadro de mandos, abra la última puntuación de riesgo, 230 según la captura de pantalla (su puntuación puede ser un poco diferente dependiendo de la actividad/reglas UBA que se hayan detectado) y observe lo siguiente:
    ...
    - Los otros eventos que no has modificado (tienen configuración por defecto) también se activan como:
      ...
      - El acceso a sitios maliciosos e ilegales, URL de riesgo, alerta de cuenta potencialmente phishing y la cuenta de usuario se utiliza desde múltiples hosts y geolocalización.
        ...
        Con tal cantidad de información se podría concluir que esta cuenta de usuario tiene acceso a un sitio malicioso, se ve comprometida y la cuenta es robada y utilizada en todo el mundo.
...
7.  Para saber a qué sitios maliciosos ha accedido el usuario, sustituyamos las métricas _Aliases_ y _Peer group deviations_ por add **URLs y URL Categories**.
...
10. Revise el sitio peachnote.com en el X-Force Exchange.
    ...
    **Nota:** X-Force ha marcado ese sitio como malicioso, lo que explica por qué se activan otros eventos/reglas UBA. Dado que la UBA de QRadar está enriquecida con información sobre amenazas de X-Force, la UBA ha indicado la URL de riesgo de acceso del usuario y la URL ilegal.
...
3.  Cierre la pestaña de búsqueda y vuelva a la tabla Cronología > Eventos y haga clic en el caso de uso **Servidor del formulario de eventos \_BlueCoat**.
...
13. Para comprobar la reputación X-Force de las URL detectadas, reformatee AQL del siguiente modo
    ...
    ```bash
    ¡SELECT url as key, **xforce_url_category(key)** FROM events WHERE ( LOWER(username) in (\'mcoy\',\'mcoy@ibmemm.edu\',\'uc_1_2_midexpected-1\') ) AND devicetype not in (16, 105, 147, 262, 267, 275, 355, 368, 500) AND islogonly=FALSE AND key is not null AND LOGSOURCETYPENAME(devicetype) != \'Custom Rule Engine\' AND LOGSOURCETYPENAME(devicetype) != \'IBM Sense\' START 1662688080000 STOP 1662688260000
    ```
...
17. Por último, puede comprobar qué direcciones IP están conectadas a su puerta de enlace VPN. En la línea de tiempo, seleccione **Log Sources**.
...
19. **Modifique** la consulta AQL por defecto como sigue
    ...
    ```bash
    SELECT LOGSOURCENAME(logsourceid) as key, sourceip FROM events WHERE ( LOWER(nombredeusuario) in (\'mcoy\',\'mcoy@ibmemm.edu\',\'uc_1_2_midexpected-1\') ) AND devicetype not in (16, ¡105, 147, 262, 267, 275, 355, 368, 500) AND islogonly=FALSE AND key = \'VPN Gateway\' AND LOGSOURCETYPENAME(devicetype) != \'Custom Rule Engine\' AND LOGSOURCETYPENAME(devicetype) != \'IBM Sense\' START 1662783840000 STOP 1662784020000
    ```
...
### Investigaciones y seguimiento del comportamiento mediante listas de vigilancia
...
- Las listas de seguimiento pueden crearse 1. **a partir de una lista de usuarios que se encuentran en un**conjunto de referencia La lista de vigilancia se actualiza a medida que se añaden o eliminan usuarios del conjunto de referencia. La lista de vigilancia de **cuentas inactivas** predeterminada se basa en el conjunto de referencia **UBA : Cuentas inactivas**, que no debe editarse ni eliminarse.
