...
6. If we still wanted to go with the **starcoder-15.5b**, we can use **Stop sequences** to eliminate the triple double-quotes. Open the **Model parameters** panel and enter `}<carriage return>}` in the Stop sequences text box. (You can type a carriage return by pressing the *Enter* or *Return* key on your keyboard.)
