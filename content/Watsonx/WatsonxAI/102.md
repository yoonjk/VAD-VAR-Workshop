...
This part 2 of the watsonx.ai L3 badge lab. In the previous lab we experiemented with different foundation models and saw how tweaks to a zero shot prompt can make a big difference in the generated output.

In this lab we will explore other techniques for getting the best results from a foundation model.

## Modifying parameters

In this lab we will continue working in the Prompt Lab, and experiment with configurable model parameters. If necessary, [navigate back to the watsonx.ai home screen](/watsonx/watsonxai/000#accessing-watsonxai-from-ibm-cloud), and enter the Prompt Lab UI

If you have already been working in the Prompt Lab, [open a new Prompt Lab session](/watsonx/watsonxai/000#creating-a-new-prompt-lab-session)
...
2. We will again start with a slightly modified prompts. Edit the **Try** section details, adding "Do not add any additional information." as the final instruction. Click **Generate**.

    You should see the same output as before, with the hallucinated url.

    ![flan-ul2_output](./images/101/flan-ul2-output.png)

3. Next we will modify some inference parameters to see the impact on the generated output. First an explanation of the configurable parameters. Open the settings panel by clicking the **Model parameters** icon shown below.
...
4. This opens a list of parameters than can be updated.  All models utilize the same inference parameters. Let's explain the purpose of each.
    ...
    **a. Greedy** vs. **Sampling Decoding** - This is how the model chooses the tokens for output. 
    ...
    The first 4 parameters below (b through e) are only visible when in **Sampling** mode.
    ...
    **c. Top P (nucleus sampling)** - a floating point number ranging from 0.0 to 1.0. At a high level, when generating a completion, a model calculates the probability of the next word based on all the previous words. **Top P** decides whether the model will always pick the most likely outcome, or allow more randomness for the next words.

    **Top P** sampling chooses from the smallest possible set of "next" words whose cumulative probability exceeds the value provided for **Top P**
    ...
    **g. Stop sequences** - sequences of characters (text, special characters, carriage return, etc) used as a stop indicator by the model. 

    The stop sequence itself will still be included in the model output, but that will be the last piece of output.
...
5. The values you see are the default values for the **Marketing email generation** sample use case. Let's play with the values and see how some changes impact our output.  Leave the current prompt in place, and change the value of **Temperature** from **0.8** to **0.4**.
    ...
    Notice by lowering the creativity the output changed quite a bit and we no longer have the website hallucination.
...
7. Now let's update the **Top K** value to 25 and reset **Top P** to **1.0**.
    ...
    The output has reverted to the original, with the url hallucination.  The model is now choosing the next token from the 25 tokens with the highest probability, so clearly the update to **Top P** resulted in fewer than 25 options for the next token.

8. Continue lowering the **Top K** value.  Notice that you'll have to lower it quite far (to about 4) to get rid of the hallucination and see output similar to what we got with the **Top P** modification. 
...
1. [Open a new Prompt Lab session](/watsonx/watsonxai/000#creating-a-new-prompt-lab-session) and select the **Freeform** tab instead of the default **Structured** tab. Choose "Switch mode" when prompted to Switch to freeform mode.
...
6. Change the model to the **mpt-7b-instruct2** model. Remove the "1." from the prompt and click **Generate**. You should see the following:

    ![list_output3](./images/102/list-output3.png)

    This looks more like what a human would expect. For this kind of request, the **mpt-7b-instruct2** model is much better than the flan models. Additionally, the model correctly interpretted that the customer was frustrated with the support system, although this was not explicitly stated. 
...
1. [Open a new Prompt Lab session](/watsonx/watsonxai/000#creating-a-new-prompt-lab-session) in the default **Structured** mode and ensure that you are using the **flan-ul2-20b** model.
...
4. Select the **mpt-7b-instruct2** model and click **Generate**. You should see the following output.

    ![json_output2](./images/102/json-output2.png)

    This is the valid JSON we are looking for. Again the smaller, mpt-7b-instruct2 model seems best for the use case. 

5. Now let's try with the **starcoder-15.5b** model, which is trained for code generation. Select this model and click **Generate**. You should see:

    ![json_output3](./images/102/json-output3.png)

    Here we have a valid JSON, but with 3 double-quotes (") at the bottom. This may be due to Python code using triple quotes to delineate multi line strings.

6. If we still wanted to go with the **starcoder-15.5b**, we can use **Stop sequences** to eliminate the triple double-quotes. Open the **Model parameters** panel and enter `}<carriage return>}` in the Stop sequences text box.
...
- We experimented with the following configuration parameters: 
    ...
    - **Top P** - lower value means less variability 
    - **Top K** - lower value means less variability
...
- It's important to pick the right model for the task at hand, which is not necessarily the larger model. The **mpt-7b-instruct2** model was best suited for both lists and JSON output.  Although smaller than the flan models, was trained by IBM to follow a user's instructions. 
