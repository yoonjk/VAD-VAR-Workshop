...
Se puede encontrar la lista de conectores más actual y los tipos de sentencias SQL soportadas [aquí ](https://www.ibm.com/docs/en/watsonxdata/1.0.x?topic=watsonxdata-supported-sql-statements). La lista de conectores soportados crecerá con el tiempo.
...
> **Nota: **: La contraseña del servidor de Db2 incorporado ha caducado en octubre de 2023. Antes de continuar con las instrucciones de esta sección, abra una ventana de mandatos de terminal y ejecute los mandatos siguientes para solucionar el problema de contraseña.

```bash
ssh -p <port#> watsonx@<hostname>
```

Cuando se le solicite la contraseña, especifique watsonx.data (no verá la contraseña tal como la escribe).
...
Con el problema de contraseña fijado, continúe con los pasos siguientes.
...
*   **Nombre de base **: GOSALES
*   **Nombre de visualización **: db2DW
*   **Nombrehost **: 192.168.252.2
*   **Puerto **: 50000
*   **Nombre **: db2inst1
*   **Contraseña **: db2inst1
...
*   **Base de datos **: PostgreSQL (se puede encontrar en Otras bases de datos)
*   **Nombre de base **: gosales
...
> **NOTA: **: Si obtiene un error 'db2inst1 password caducado' al intentar conmutar el 'db2catalog' y la información de esquema y tabla no aparece, los pasos siguientes le ayudarán a remediarlo:
>
> 1.  Vuelva a la página Gestor de infraestructura y desconecte el inicio de db2catalog
> 2.  Pase el ratón por encima del mosaico db2catalog y pulse el icono de trashcan para eliminarlo.
> 3.  SSH en la instancia de watsonx en la línea de mandatos y cambie al usuario root. ( `sudo su -`)
> 4.  Ejecute el siguiente mandato Docker para ampliar la caducidad de la contraseña: `docker exec db2server chage -I -1 -m 0 -M 99999 -E -1 db2inst1`
> 5.  Vuelva y siga los pasos 2-6 para volver a añadir la base de datos db2.
...
Si no ve ninguna tabla asociada con el **gosalesdw **en cualquiera de los catálogos, puede renovar el esquema.
